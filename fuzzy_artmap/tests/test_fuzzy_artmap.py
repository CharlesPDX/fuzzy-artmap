...
in_class = np.array([[1,0]])
training_in_value = np.array([[0.1,0.1,0.9,0.9]])
test_in_value = np.array([[0.11,0.11,0.99,0.99]])

out_class = np.array([[0,1]])
training_out_value = np.array([[0.9,0.9,0.1,0.1]])
test_out_value = np.array([[0.99,0.99,0.11,0.11]])
...
def test_fuzzy_artmap_selects_first_uncommited_node_with_wide_variance_input() -> None:
    ...
    learned_a_weight = np.copy(fuzzy_artmap._weight_a[0, None])
    learned_ab_weight = np.copy(fuzzy_artmap._weight_ab[0, None])
    ...
    assert np.array_equal(learned_a_weight, first_category_a_weight)
    assert np.array_equal(learned_ab_weight, first_category_ab_weight)
...
def test_fuzzy_artmap_predicts_training_value() -> None:
    ...
    fuzzy_artmap.fit(np.vstack([training_in_value, training_out_value]), np.vstack([in_class, out_class]))
    
    predicted_class = fuzzy_artmap.predict(training_in_value)
    assert np.array_equal(in_class, predicted_class[0])

    predicted_class = fuzzy_artmap.predict(training_out_value)
    assert np.array_equal(out_class, predicted_class[0])
...
def test_fuzzy_artmap_grows_nodes() -> None:
    ...
    fuzzy_artmap.fit(np.vstack([training_in_value, training_out_value, test_in_value, test_out_value]), np.vstack([in_class, out_class, in_class, out_class]))
...
def test_calculate_first_category_choice() -> None:
    ...
    category_choice_numerator = np.sum(training_in_value, 1)

    expected_match_function = category_choice_numerator / np.sum(training_in_value, 1)
    ...
    expected_category_choice_function = category_choice_numerator / (fuzzy_artmap._choice_parameter + np.sum(np.ones((1, 4)), 1))
    ...
    assert np.array_equal(match_function, expected_match_function)
    assert np.array_equal(category_choice_function, expected_category_choice_function)
...
def test_fuzzy_artmap_matches_expected_learning() -> None:
    ...
    assert np.array_equal(fuzzy_artmap._weight_a, complement_encoded_first_point)
    assert np.array_equal(fuzzy_artmap._weight_ab, in_class)
    ...
    assert np.allclose(fuzzy_artmap._weight_a, np.array([[0.1000, 0.1000, 0.8925, 0.9000]]))
    assert np.array_equal(fuzzy_artmap._weight_ab, in_class)
    ...
    assert np.allclose(fuzzy_artmap._weight_a, np.array([[0.1000, 0.1000, 0.875625, 0.9000]]))
    assert np.array_equal(fuzzy_artmap._weight_ab, in_class)
    ...
    assert np.allclose(fuzzy_artmap._weight_a[0, None], np.array([[0.1000, 0.1000, 0.875625, 0.9000]]))
    assert np.array_equal(fuzzy_artmap._weight_ab[0, None], in_class)

    assert np.allclose(fuzzy_artmap._weight_a[1, None], np.array([[0.14, 0.1 , 0.86, 0.9 ]]))
    assert np.array_equal(fuzzy_artmap._weight_ab[1, None], out_class)
